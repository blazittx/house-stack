...
const buildEmailHtml = ({ amount, perPerson, swishNumber, swishName, swishMessage, swishLink }) => {
  ...
  const paySection = swishLink
    ? `<a href="${escapeHtml(swishLink)}" style="display:inline-block;padding:12px 18px;border-radius:8px;background:#0b0b0b;color:#ffffff;text-decoration:none;border:1px solid #222222;">Pay with Swish</a>`
    : `<span style="color:#666666;">Open Swish and pay ${safePerPerson || safeAmount} SEK to ${safeNumber}.</span>`

  return `
    <div style="font-family:Arial,sans-serif;line-height:1.5;color:#111111;">
      <h2 style="margin:0 0 8px;">Swish payment request</h2>
      <p style="margin:0 0 12px;">You have a new request from ${safeName || 'your housemate'}.</p>
      <table style="border-collapse:collapse;margin:12px 0;">
        <tr><td style="padding:4px 12px 4px 0;color:#666;">Total</td><td style="padding:4px 0;">${safeAmount || '—'} SEK</td></tr>
        <tr><td style="padding:4px 12px 4px 0;color:#666;">Split (3)</td><td style="padding:4px 0;">${safePerPerson || '—'} SEK</td></tr>
        <tr><td style="padding:4px 12px 4px 0;color:#666;">Swish number</td><td style="padding:4px 0;">${safeNumber || '—'}</td></tr>
        <tr><td style="padding:4px 12px 4px 0;color:#666;">Message</td><td style="padding:4px 0;">${safeMessage || '—'}</td></tr>
      </table>
...
exports.handler = async (event) => {
  ...
  const text = [
    ...
    Number.isFinite(perPerson) ? `Split (3): ${perPerson.toFixed(2)} SEK` : null,
